...
country_names <- unique(economic_inequality$Country)
inequality_indicator <- unique(economic_inequality$MEASURE)[-3]
...
all_indicators <-
  purrr::map(country_names, function(country_name) {
    purrr::map(inequality_indicator, function(inequality_indicator) {
      ...
      specific_country <- with(economic_inequality,
                               economic_inequality[MEASURE == inequality_indicator &
...
write_csv(all_ineq_indicators, "./data-raw/raw_economic_inequality.csv")
devtools::use_data(economic_inequality, overwrite = TRUE)
